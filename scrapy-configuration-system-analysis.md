...
#### ⚠️ 重要说明：执行顺序 vs 优先级顺序

**本图展示的是执行顺序（按时间先后），不是优先级顺序（按数值高低）！**

| 对比维度 | 执行顺序（本图展示） | 优先级顺序（覆盖规则） |
|----------|----------------------|------------------------|
| 含义 | 配置被注入的时间先后顺序 | 决定配置覆盖关系的数值 |
| 关键规则 | 先执行的配置可能被后执行的配置覆盖 | 优先级数值高的覆盖数值低的 |
| 特殊情况 | `command` (10) 在 `project` (20) 之后执行 | `command` (10) 不会覆盖 `project` (20)，因为 10 < 20 |

**核心原则**：
1. **优先级数值是决定覆盖关系的唯一标准**，与执行顺序无关
2. 即使 `command` (10) 在 `project` (20) 之后执行，也不会覆盖 `project` 配置
3. 插件如果不显式指定 `priority="addon"`，会以 `priority=20` 执行，可以覆盖先执行的 `project` 配置（同级即覆盖）
