...
从 URI 设计的根本原则来看，**URI 的核心作用是作为资源的唯一标识符**。即使服务器对 `/path` 和 `/path/` 返回相同的内容，它们在语义上代表不同的标识符，爬虫应该尊重这种区别而不是强制统一。
...
本节补充解释 `canonicalize_url` 中查询参数用 `+` 而不是 `%20` 的协议依据（路径规范化和尾斜杠保留的协议依据已在 1.2.3 节详细说明）。
